...
class Food(Thing):
    ...
    def eaten(self):
        give = np.random.uniform(0, min(self.MAX_GIVE, self.energy))
        self.energy -= give
        return give
...
class Cell(Thing):
    ...
    def reproduce(self):
        ...
        reproduction_threshold = np.sqrt(self.energy) + 1
        baby_energy = max(1, reproduction_threshold)
        
        reproduction_energy = baby_energy ** self.TOTAL_ENERGY_POWER
        
        while self.energy > reproduction_energy + reproduction_threshold: 
